...
import { changePreset, checkIfThemeExists, generatePreset, toast } from "../backend";
...
export function CreatePresetModal({ closeModal }: { closeModal: () => void }) {
  ...
  async function createPreset(input: string) {
    if (input) {
      ...
      await changePreset(backendManifestVersion >= 9 ? input + ".profile" : input, localThemeList);
      await refreshThemes();
