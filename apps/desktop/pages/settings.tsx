...
export default function SettingsPage() {
  ...
  async function updateBetaTranslations(value: boolean) {
    ...
    try {
      const res = await setBetaTranslations(value);
      const applied =
        res.success &&
      ...
      if (!applied) {
        toast("Setting saved, but CSS translations could not be downloaded.");
        return;
      }

      await refreshThemes(false);
      toast("CSS translations downloaded and the active profile was rebuilt.");
    } catch (error) {
      toast(`Could not update CSS translations: ${String(error)}`);
    } finally {
      setOngoingAction(false);
    }
  }

  async function waitForBackend(attempts: number = 20) {
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      const result = await dummyFunction();
      if (result && result.success && result.result) return true;
      await sleep(500);
    }
    return false;
  }
  ...
  async function repairBackend() {
    ...
    try {
      ...
      if (String(startResult).startsWith("ERROR")) {
        toast(String(startResult));
        return;
      }

      if (!(await waitForBackend())) {
        toast("The backend did not respond after it was restarted.");
        return;
      }

      await refreshThemes(false);
      toast("Backend repaired and connected.");
    } catch (error) {
      toast(`Could not repair the backend: ${String(error)}`);
    } finally {
      setOngoingAction(false);
    }
  }
          ...
          <div className="flex w-full flex-col gap-4">
            ...
            {isWindows && (
              <>
                ...
                >
                  {ongoingAction ? <ImSpinner5 /> : "Repair / Restart Backend"}
                </button>
                <button
                  disabled={ongoingAction}
                  onClick={async () => {
