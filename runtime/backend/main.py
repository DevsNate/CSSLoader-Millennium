import os, asyncio, sys, time, aiohttp, json, socket, ssl, certifi
...
from css_utils import Log, create_steam_symlink, Result, get_theme_path, store_read as util_store_read, store_write as util_store_write, store_or_file_config, is_steam_beta_active, use_millennium_theme_runtime
...
async def fetch_class_mappings(css_translations_path : str, loader : Loader):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=tls_context, use_dns_cache=False), timeout=aiohttp.ClientTimeout(total=30)) as session:
            async with session.get(css_translations_url) as response:
                if response.status == 200:
                    ...
                    await loader.reset(silent=True)
                    return True
...
class Plugin:
    ...
    async def change_preset(self, name : str) -> dict:
        return (await self.loader.change_preset(name)).to_dict()
    ...
    async def fetch_class_mappings(self):
        success = await self._fetch_class_mappings(self)
        return Result(success, "Success" if success else "Failed to download CSS translations").to_dict()

    async def set_beta_translations(self, enabled : bool) -> dict:
        util_store_write("beta_translations", "1" if enabled else "0")
        success = await self._fetch_class_mappings(self)
        return Result(
            success,
            "Success" if success else "Setting saved, but CSS translations could not be downloaded"
        ).to_dict()
    ...
    async def _fetch_class_mappings(self, run_in_bg : bool = False):
        ...
        if run_in_bg:
            ...
            return await fetch_class_mappings(css_translations_path, self.loader)
