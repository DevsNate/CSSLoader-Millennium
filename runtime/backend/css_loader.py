from css_utils import Log, Result, get_theme_path, FLAG_KEEP_DEPENDENCIES, FLAG_PRESET, use_millennium_theme_runtime
...
class Loader:
    def __init__(self):
        ...
        self.millennium_theme_mode = use_millennium_theme_runtime()
    ...
    async def set_theme_state(self, name : str, state : bool, set_deps : bool = True, set_deps_value : bool = True) -> Result:
        ...
        try:
            if state:
                result = await self._enable_theme(theme, set_deps, set_deps_value, inject_now=not self.millennium_theme_mode)
            else:
                result = await self._disable_theme(theme, FLAG_KEEP_DEPENDENCIES in theme.flags, remove_now=not self.millennium_theme_mode)

            await self.commit_runtime()
            return result
    ...
    async def change_preset(self, name : str) -> Result:
        ...
        try:
            preset = None
            if name != "None":
                preset = await self._get_theme(name)
                if preset is None:
                    return Result(False, f"Did not find profile {name}")
                if FLAG_PRESET not in preset.flags:
            ...
            for theme in self.themes:
                if theme.enabled:
                    await theme.remove(remove_now=not self.millennium_theme_mode)

            result = Result(True)
            if preset is not None:
                result = await self._enable_theme(
                    preset,
                    set_deps=True,
                    set_deps_value=True,
                    inject_now=not self.millennium_theme_mode,
                )
    ...
    async def set_patch_of_theme(self, themeName : str, patchName : str, value : str) -> Result:
        ...
        if (themePatch.theme.enabled):
            await themePatch.remove(remove_now=not self.millennium_theme_mode)
            await themePatch.inject(inject_now=not self.millennium_theme_mode)
    ...
    async def set_component_of_theme_patch(self, themeName : str, patchName : str, componentName : str, value : str) -> Result:
        ...
        if self.millennium_theme_mode:
            ...
            if result.success and component.inject.enabled:
                # Decky removes and reinserts an active component style, which
                # moves the updated CSS variable to the end of the cascade.
    ...
    async def delete_theme(self, themeName : str) -> Result:
        ...
        result = await theme.delete(remove_now=not self.millennium_theme_mode)
        if not result.success:
    ...
    async def _enable_theme(self, theme : Theme, set_deps : bool = True, set_deps_value : bool = True, ignore_dependencies : list = [], inject_now : bool = True) -> Result:
        if theme is None:
            return Result(False)
            ...
            for dependency_name in theme_dependencies:
                ...
                if set_deps_value:
                    if dependency.enabled:
                        await dependency.remove(remove_now=inject_now)
                ...
                await self._enable_theme(dependency, set_deps, set_deps_value, ignore_dependencies_next, inject_now)
    ...
    async def _disable_theme(self, theme : Theme, keep_dependencies : bool, remove_now : bool = True) -> Result:
        ...
        result = await theme.remove(remove_now)
        ...
        for dependency_name in theme.dependencies:
            ...
            if not used:
                await self._disable_theme(dependency, False, remove_now)
