...
def use_millennium_theme_runtime() -> bool:
    runtime = os.getenv("CSS_LOADER_RUNTIME", "").strip().lower()
    if runtime == "cdp":
        return False
    if runtime == "millennium-theme":
        return True

    return PLATFORM_WIN and os.path.isdir(os.path.join(get_steam_path(), "millennium"))
